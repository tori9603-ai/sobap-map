...
# ⚠️ 사장님의 정보 (구글 시트 순서: owner, address, lat, lon, bbox)
...
def get_data():
    try:
        ...
        if len(data) > 1:
            ...
            # 💡 모든 데이터를 강제로 글자로 변환하여 AttributeError 원천 차단
            ...
            return df
        return pd.DataFrame(columns=['owner', 'address', 'lat', 'lon', 'bbox'])
    except:
        return pd.DataFrame(columns=['owner', 'address', 'lat', 'lon', 'bbox'])
...
def get_kakao_location(query):
    ...
    # 1차 주소 검색
    res = requests.get(f"https://dapi.kakao.com/v2/local/search/address.json?query={query}", headers=headers)
    
    if res.status_code == 403:
        return [], "❌ [도메인 불일치] 카카오 설정을 확인하세요."
    
    data = res.json()
    if data.get('documents'): return data['documents'], "✅ 성공"
    
    # 2차 키워드 검색
    res_kw = requests.get(f"https://dapi.kakao.com/v2/local/search/keyword.json?query={query}", headers=headers).json()
    return res_kw.get('documents', []), "✅ 키워드 검색"
...
with st.sidebar:
    ...
    unique_owners = sorted(df['owner'].unique().tolist())
    ...
    if selected_owner != "선택":
        ...
        for idx, row in owner_data.iterrows():
            ...
            with c1:
                if st.button(f"🏠 {place_display if 'place_display' in locals() else place_name}", key=f"mv_{idx}"):
        ...
        search_addr = st.text_input("상세 주소 또는 건물명")
