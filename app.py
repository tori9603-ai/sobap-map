...
# ⚠️ 사장님의 URL과 카카오 API 키 (A:owner, B:address, C:lat, D:lon 순서)
...
def get_data():
    try:
        ...
        if len(data) > 1:
            ...
            # 💡 [데이터 수정] 모든 데이터를 강제로 글자로 변환하여 '0' 오류 방지
            df['owner'] = df['owner'].astype(str).str.strip()
            df['lat'] = pd.to_numeric(df['lat'], errors='coerce').fillna(0)
            df['lon'] = pd.to_numeric(df['lon'], errors='coerce').fillna(0)
            # 이름이 '0'이거나 비어있는 무의미한 데이터 제외
...
# 💡 [검색 엔진] 주소 검색 실패 시 키워드 검색으로 자동 전환
def get_kakao_location(query):
    ...
    # 2차: 주소 검색 결과 없으면 키워드(건물/지번) 검색 시도
...
with st.sidebar:
    ...
    with st.expander("➕ 신규 점주 등록"):
        ...
        if st.button("구글 시트에 영구 등록"):
            if add_name:
                # 💡 [순서 주의] owner, address, lat, lon 순서로 전송
    ...
    if selected_owner != "선택":
        ...
        search_addr = st.text_input("상세 주소, 지번, 또는 건물명")
        ...
        if st.button("🔍 카카오 정밀 검색"):
            ...
            if results:
                ...
                st.warning("결과가 없습니다. 주소를 더 정확히 입력해 주세요.")
